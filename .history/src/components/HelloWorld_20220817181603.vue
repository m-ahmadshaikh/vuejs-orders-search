...
start_date: moment("2000-11-07T09:00:13Z"),
  end_date: moment("2220-11-07T09:00:13Z"),
   ...
   computed: {
    ...
    (this.order_products[order['id']][0].toString().toLowerCase().includes(this.searchQuery.toLowerCase())|| this.order_products[order['id']][0].toString().toLowerCase().includes(this.searchQuery.toLowerCase())order.order_name.toString().toLowerCase().includes(this.searchQuery.toLowerCase()  ) && (order.check_date.isAfter(this.start_date)  && order.check_date.isBefore(this.end_date)) )
  );
      }else{
