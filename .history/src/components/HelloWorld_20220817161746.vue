...
export default {
  ...
  .then((data) => {
	...
	var date  = moment(item.created_at).utc().format('LL, hh:mm a');
                this.orders.push({ id: item.id, order_name: item.order_name, created_at: date, customer_id: item.customer_id,check_date : });
        });
  ...
  end_date: moment("2220-11-07T09:00:13Z"),
   ...
   computed: {
    ...
    order.order_name.toString().toLowerCase().includes(this.searchQuery.toLowerCase() && order.created_at >= this.start_date && order.created_at <= this.end_date )
        ...
        return this.orders.filter((order) => order.created_at.isAfter(this.start_date)  && order.created_at.isBefore(this.end_date)  );
      }
    }
  },
