...
export default {
  ...
  .then((data) => {
    ...
    // Getting every object from the array and pushing the names and images to OUR array
		...
		response = await fetch(`https://orders-go.herokuapp.com/order_items`)
        .then((data) => {
    this.order_items   = [];
    // Getting every object from the array and pushing the names and images to OUR array
                data.forEach(item => {
                this.order_items.push({ id: item.id, order_id: item.order_id, price_per_unit: item.price_per_unit, customer_id: item.customer_id });
        });
