...
<template>
	<div class="home">
		...
		<div class="col-md-6 centeralign">
			...
			<div class="card-body">
				<div align = "center">
      <table v-bind:orderinfo = "orderinfo">
        <tr>
            <th>Order Name</th>
        ...
        <tr v-for="place in filteredList()" :key="place.id">
  ...
  .then((data) => {
	...
	methods: {
		...
		getLocationDetails() {
			...
			var filteredData = this.orders.filter(function(obj) {
	return obj['order_name'] === this.input.ordername;
});
...
function filteredList() {
  return this.orders.filter((order) =>
    order.toLowerCase().includes(input.value.toLowerCase())
  );
}
</script>
<style scoped>
