	...
	<div class="home">
		...
		<div >
			...
			<input class="col-md-6 centeralign" type="text" style="width:400px;"
				v-model="searchQuery"  
				placeholder="Search by Order Name or Product Name" />
...
start_date: moment("2000-11-07T09:00:13Z"),
  end_date: moment("2220-11-07T09:00:13Z"),
   ...
   computed: {
    ...
    (this.order_products[order['id']][0].toString().toLowerCase().includes(this.searchQuery.toLowerCase())|| this.order_products[order['id']][1].toString().toLowerCase().includes(this.searchQuery.toLowerCase())|| order.order_name.toString().toLowerCase().includes(this.searchQuery.toLowerCase()  ) && (order.check_date.isAfter(this.start_date)  && order.check_date.isBefore(this.end_date)) )
  );
      }else{
