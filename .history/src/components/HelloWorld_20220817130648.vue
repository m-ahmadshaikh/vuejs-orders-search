...
export default {
  ...
  .then((data) => {
    ...
    // Getting every object from the array and pushing the names and images to OUR array
                data.forEach(item => {
                this.customers.push({ user_id: item.user_id, login: item.login, password: item.password, name: item.name, company_id: item.company_id, credit_cards: item.credit_cards });
        });
	
		console.log(this.customers[0].user_id)
  });
  ...
  .then((data) => {
    ...
    // Getting every object from the array and pushing the names and images to OUR array
                data.forEach(item => {
                this.customer_companies.push({ company_id: item.company_id, order_item_id: item.order_item_id, delivered_quantity: item.delivered_quantity});
        });
