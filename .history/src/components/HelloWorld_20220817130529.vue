...
export default {
  ...
  .then((data) => {
    ...
    // Getting every object from the array and pushing the names and images to OUR array
                data.forEach(item => {
                this.customers.push({ user_id: item.user_id, login: item.login, password: item.password, name: item.name, company_id: item.company_id, credit_cards: items.credit_cards });
        });
