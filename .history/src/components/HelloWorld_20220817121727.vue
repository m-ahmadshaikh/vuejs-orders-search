...
export default {
	...
	mounted(){
       var response =  axios.get(`https://orders-go.herokuapp.com/orders`)
        this.countrydetails = response.d
...
var arr = this.companydetails.map(function(item) {
	return new OrderModel(item.id, item.created_at, item.ordername, item.customer_id)});
console.log(arr);
