	...
	<div class="home">
		...
		<div >
			...
			<input class="col-md-6 centeralign" type="text" style="width:400px;"
				v-model="searchQuery"  v-on:input="getLocationDetails()"
			...
			<input v-model="input.start_date" type="date" name="startDate" placeholder="Start date" />
&nbsp;
			<label for="endDate">End Date</label>
            <!-- The end date field -->
            <input v-model="input.end_date" type="date" name="endDate" placeholder="End date" /><br><br>
			<div class="card-body">
				<div align = "center">
				
      <table class="col-md-6 centeralign" v-bind:orderinfo = "orderinfo">
        <tr>
            <th>Order Name</th>
        ...
        <tr v-for="place in res" :key="place.id">
...
export default {
  ...
  .then((data) => {
	...
	var date  = moment(item.created_at).utc().format('LL, hh:mm a');
                this.orders.push({ id: item.id, order_name: item.order_name, created_at: date, customer_id: item.customer_id });
        });
...
searchQuery: null,
   ...
   computed: {
    ...
    order.order_name.toString().toLowerCase().includes(this.searchQuery.toLowerCase())
  );
      }else{
        return this.orders;
      }
