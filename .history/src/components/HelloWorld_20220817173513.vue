...
export default {
  ...
  .then((data) => {
    ...
    // Getting every object from the array and pushing the names and images to OUR array
                data.forEach(item => {
            this.order_items.hasOwnProperty(item_order_id)?      this.order_items[item.order_id] =  parseFloat(item.quantity) *  parseFloat( item.price_per_unit !== null? item.price_per_unit:0): ;
        });
