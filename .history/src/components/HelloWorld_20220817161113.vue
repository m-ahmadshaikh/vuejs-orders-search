...
export default {
  ...
  .then((data) => {
	...
	data() {
		...
		return {
searchQuery: null,
start_date: "2000-11-07T09:00:13Z",
  end_date: "2220-11-07T09:00:13Z",
   ...
   computed: {
    ...
    order.order_name.toString().toLowerCase().includes(this.searchQuery.toLowerCase() && order.created_at >= this.start_date && order.created_at <= this.end_date )
  );
      }else{
        return this.orders.filter((order) => order.created_at >= this.start_date && order.created_at <= this.end_date );
      }
    }
  },
