  ...
  .then((data) => {
    ...
    // Getting every object from the array and pushing the names and images to OUR array
            ...
            Object.hasOwn(this.order_items,item.order_id) != true?   
               this.order_items[item.order_id] =  parseFloat(item.quantity).toFixed(2) *  parseFloat( item.price_per_unit !== null? item.price_per_unit.toFixed(2):0) :
               this.order_items[item.order_id] +=  parseFloat(item.quantity).toFixed(2) *  parseFloat( item.price_per_unit !== null? item.price_per_unit:0).toFixed(2)
            ;
        });
