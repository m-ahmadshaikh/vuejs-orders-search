...
export default {
	...
	async mounted(){
       var response = await axios.get(`https://orders-go.herokuapp.com/orders`)
		...
		response = await axios.get(`https://orders-go.herokuapp.com/order_items`)
        this.order_items = response.data;
		var jsonParsedArray = JSON.parse(this.order_items);
		for (let key in jsonParsedArray) {
		if (jsonParsedArray.hasOwnProperty.call(entries, "key)) {
