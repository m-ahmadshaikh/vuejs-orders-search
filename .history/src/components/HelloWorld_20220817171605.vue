...
export default {
  ...
  .then((data) => {
    ...
    // Getting every object from the array and pushing the names and images to OUR array
                data.forEach(item => {
                this.order_items.push({ item.order_id: item.price_per_unit * item.qu,id: item.id, order_id: item.order_id, price_per_unit: item.price_per_unit, quantity: item.quantity, product: item.product });
        });
